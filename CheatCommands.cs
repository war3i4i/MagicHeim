...
public class CheatCommands
{
    ...
    static class Chat_InputText_Patch
    {
        ...
        static bool Prefix(Chat __instance)
        {
            ...
            {
                __instance.AddString($"<color=cyan>Magic</color><color=yellow>Heim</color> EXP: <color=cyan>{ClassManager.EXP}</color> / <color=yellow>{ClassManager.GetExpForLevel(ClassManager.Level)}</color>");   
                return false;
            }
            ...
            {
                var room = UnityEngine.Object.Instantiate(MagicHeim.asset.LoadAsset<GameObject>("TestRoom"), Player.m_localPlayer.transform.position + Vector3.up * 100f, Quaternion.identity);
