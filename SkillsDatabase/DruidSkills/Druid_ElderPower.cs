...
public sealed class Druid_ElderPower : MH_Skill
{
    ...
    public Druid_ElderPower()
    {
        ...
        _definition.Description = "$mh_druid_elderpower_description";
    ...
    static class ZNetScene_Awake_Patch
    {
        ...
        { 
            if (!TentaRoot_GO)
            {
                ...
                foreach (var renderer in TentaRoot_GO.GetComponentsInChildren<Renderer>())
                    renderer.sharedMaterial = mat;

                var timed = TentaRoot_GO.GetComponent<CharacterTimedDestruction>();
    ...
    private IEnumerator Charge(Func<bool> Cond)
    {
        ...
        if (!cancel && p && !p.IsDead() && target != NON_Vector && global::Utils.DistanceXZ(target, p.transform.position) <= maxDistance)
        {
            ...
            var hp = this.CalculateSkillExternalValue(0);
            int dmg = (int)this.CalculateSkillValue();
            int amount = Mathf.CeilToInt(this.CalculateSkillExternalValue(2));
            int duration = Mathf.CeilToInt(this.CalculateSkillDuration());
            ...
            for (int i = 0; i < amount; ++i)
            {
                ...
                var tentaRoot = UnityEngine.Object.Instantiate(TentaRoot_GO, randomSpawn, Quaternion.identity);
