...
public sealed class Druid_WeaponEnchantSpirit : MH_Skill
{
    private static GameObject VFX_Fist;
    private static GameObject VFX;
    private static GameObject VFX_NonRead;

    public Druid_WeaponEnchantSpirit()
    {
        _definition._InternalName = "Druid_Weaponenchantspirit";
        _definition.Name = "$mh_druid_weaponenchantspirit";
        _definition.Description = "$mh_druid_weaponenchantspirit_desc";
        ...
        _definition.Icon = MagicHeim.asset.LoadAsset<Sprite>("Druid_WeaponEnchantSpirit_Icon");
        ...
        VFX = MagicHeim.asset.LoadAsset<GameObject>("Druid_WeaponEnchantSpirit_VFX");
        VFX_NonRead = MagicHeim.asset.LoadAsset<GameObject>("Druid_WeaponEnchantSpirit_NonRead");
        VFX_Fist = MagicHeim.asset.LoadAsset<GameObject>("Druid_WeaponEnchantSpirit_Fist");
        MH_WeaponEnchants_VFXs.WeaponEnchantVFXs.Add(MH_WeaponEnchant.Type.Spirit, VFX_NonRead);
    ...
    public override void Execute(Func<bool> Cond)
    {
        ...
        data.type = MH_WeaponEnchant.Type.Spirit;
    ...
    private static class MockLeft
    {
        ...
        private static void Postfix(VisEquipment __instance)
        {
            ...
            {
                if (__instance.m_nview.m_zdo.GetInt("mh_mage_weaponenchantLeft") == (int)MH_WeaponEnchant.Type.Spirit)
                {
    ...
    private static class MockRight
    {
        ...
        private static void Postfix(VisEquipment __instance)
        {
            ...
            {
                if (__instance.m_nview.m_zdo.GetInt("mh_mage_weaponenchantRight") == (int)MH_WeaponEnchant.Type.Spirit)
                {
                    GameObject go = UnityEngine.Object.Instantiate(
    ...
    public override string BuildDescription()
    {
        ...
        builder.AppendLine($"Bonus Spirit Damage: {Math.Round(currentValue, 1)}");
        ...
        if (Level < maxLevel && Level > 0)
        {
            ...
            builder.AppendLine("\nNext Level:");
            builder.AppendLine($"Bonus Spirit Damage: {Math.Round(nextValue, 1)} <color=green>({(roundedValueDiff > 0 ? "+" : "")}{roundedValueDiff})</color>");
