...
public sealed class Mage_ArcaneShield : MH_Skill
{
    private static GameObject ArcaneShield_Buff;

    public Mage_ArcaneShield()
    {
        _definition._InternalName = "Mage_Arcaneshield";
        _definition.Name = "$mh_mage_arcaneshield";
        _definition.Description = "$mh_mage_arcaneshield";

        _definition.MinLvlValue = MagicHeim.config($"{_definition._InternalName}",
            "MIN Lvl Armor Bonus", 2.5f,
            "Armor Bonus amount (Min Lvl)");

        _definition.MaxLvlValue = MagicHeim.config($"{_definition._InternalName}",
            "MAX Lvl Armor Bonus", 35f,
        ...
        _definition.Icon = MagicHeim.asset.LoadAsset<Sprite>("Mage_ArcaneShield_Icon");
        ...
        ArcaneShield_Buff = MagicHeim.asset.LoadAsset<GameObject>("Mage_ArcaneShield_Prefab");
    ...
    static class ZNetScene_Awake_Patch
    {
        ...
        {
            __instance.m_namedPrefabs[ArcaneShield_Buff.name.GetStableHashCode()] = ArcaneShield_Buff;
        }
    }
    ...
    public override void Execute(Func<bool> Cond)
    {
        ...
        int armorBonus = (int)this.CalculateSkillValue();
        ...
        foreach (Player player in players)
        {
            ...
            player.GetSEMan().AddStatusEffect("Mage_ArcaneShield_Buff".GetStableHashCode(), true, armorBonus, duration);
        }
    ...
    {
        return "<color=red>Buff, Ranged Attack Resistance</color>";
    }
    ...
    public override string BuildDescription()
    {
        ...
        builder.AppendLine($"Ranged Damage Resistance: {Math.Round(currentValue, 1)}%");
        ...
        if (Level < maxLevel && Level > 0)
        {
            ...
            builder.AppendLine("\nNext Level:");
            builder.AppendLine(
                $"Ranged Damage Resistance: {Math.Round(nextValue, 1)}% <color=green>({(roundedValueDiff > 0 ? "+" : "")}{roundedValueDiff})</color>");
            builder.AppendLine(
    ...
    public class SE_Mage_ArcaneShield_Buff : StatusEffect
    {
        public int armorBonus;

        public SE_Mage_ArcaneShield_Buff()
        {
            name = "Mage_ArcaneShield_Buff";
        ...
        public override void OnDamaged(HitData hit, Character attacker)
        {
            if (hit.m_ranged)
            {
                hit.ApplyModifier(Mathf.Clamp01(1 - armorBonus / 100f));
            }
        }
    }
    ...
    public static class Mage_ThunderSock_DB_Patches
    {
        private static void Add_SE(ObjectDB odb)
        {
            ...
            if (!odb.m_StatusEffects.Find(se => se.name == "Mage_ArcaneShield_Buff"))
                odb.m_StatusEffects.Add(ScriptableObject.CreateInstance<SE_Mage_ArcaneShield_Buff>());
        }
    ...
    public override Color SkillColor => new Color(1f, 0.27f, 0.98f);
}